...
import { UploadFile, UploadFolder } from "../types/types";
...
interface FileExplorerState {
  ...
  setUploadData: (data: UploadFolder | null) => void;
  setEditorContent: (content: string) => void;
  setOpenFiles: (files: OpenFile[]) => void;
  setActiveFileId: (fileId: string | null) => void;
  openFile: (file: UploadFile) => void;
  ...
  handleAddFile: (
    ...
    instance: any,
    saveTemplateData: (data: UploadFolder) => Promise<void>
  ) => Promise<void>;
  handleAddFolder: (
...
interface OpenFile extends UploadFile {
  id: string;
...
export const useFileExplorer = create<FileExplorerState>((set, get) => ({
  ...
  openFile: (file) => {
    const fileId = generateFileId(file, get().uploadData!);
    const { openFiles } = get();
  ...
  handleAddFile: async (newFile, parentPath, writeFileSync, instance, saveTemplateData) => {
    const { uploadData } = get();
    if (!uploadData) return;

    try {
      const updatedTemplateData = JSON.parse(JSON.stringify(uploadData)) as UploadFolder;
      ...
      set({ uploadData: updatedTemplateData });
  ...
  handleDeleteFile: async (file, parentPath, saveTemplateData) => {
    const { uploadData, openFiles } = get();
    if (!uploadData) return;
    ...
    try {
      ...
      const fileId = generateFileId(file, uploadData);
  ...
  handleDeleteFolder: async (folder, parentPath, saveTemplateData) => {
    ...
    try {
      ...
      const closeFilesInFolder = (folder: UploadFolder, currentPath: string = "") => {
        folder.items.forEach((item) => {
          if ("filename" in item) {
            // Generate the correct file ID using the same logic as openFile
            const fileId = generateFileId(item, uploadData);
            get().closeFile(fileId);
  ...
  handleRenameFile: async (
    ...
    const { uploadData, openFiles, activeFileId } = get();
    if (!uploadData) return;

    // Generate old and new file IDs using the same logic as openFile
    const oldFileId = generateFileId(file, uploadData);
    const newFile = { ...file, filename: newFilename, fileExtension: newExtension };
    const newFileId = generateFileId(newFile, uploadData);
    ...
    try {
      ...
      if (fileIndex !== -1) {
        ...
        set({
          uploadData: updatedTemplateData,
