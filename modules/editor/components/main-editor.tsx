...
interface PlaygroundEditorProps {
  activeFile: UploadFile | undefined
  content: string
  onContentChange: (value: string) => void
  suggestion?: string | null
  suggestionLoading?: boolean
  lintResults?: LintMessage[]
  ...
  onExitDiff?: () => void
  onApplyAllChanges?: (code: string) => void
...
export const PlaygroundEditor = ({
  ...
  console.log("Rendering PlaygroundEditor with props:", showDiff, modifiedCode);
  ...
  const [buttonContainers, setButtonContainers] = useState<HTMLElement[]>([])
  ...
  const getMonacoSeverity = (type: "error" | "warning" | "info" | "note" | "supplemental") => {
    if (!monacoRef.current) return undefined;
    switch (type) {
      case "error": return monacoRef.current.MarkerSeverity.Error
      case "warning": return monacoRef.current.MarkerSeverity.Warning
      case "info": return monacoRef.current.MarkerSeverity.Info
      default: return monacoRef.current.MarkerSeverity.Hint
    }
  }
  ...
  const markers = lintResults.map(msg => ({
    startLineNumber: msg.line,
    startColumn: 1,
    endLineNumber: msg.line,
    endColumn: model.getLineMaxColumn(msg.line),
    message: msg.desc,
    severity: getMonacoSeverity(msg.type),
  }));

  monacoRef.current.editor.setModelMarkers(model, "lint", markers);
}, [lintResults]);
  ...
  const acceptCurrentSuggestion = useCallback(() => {
    ...
    try {
      ...
      const range = new monacoRef.current!.Range(suggestionPos.line, suggestionPos.column, suggestionPos.line, suggestionPos.column)
  ...
  <div className="absolute top-3 right-3 z-20 flex gap-2">
    <Button
      ...
      onClick={() => {
        ...
        const modified = diffEditorRef.current
          .getModifiedEditor()
          .getValue()
        onApplyAllChanges?.(modified)
      }}
    >
      Apply Changes
    </Button>

    <Button size="sm" variant="outline" onClick={onExitDiff}>
      Cancel
    </Button>
  </div>
)}
