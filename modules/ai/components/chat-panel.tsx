...
import { EnhancedFilePreview } from "./attach-preview";
...
const MessageTypeIndicator: React.FC<{
  ...
  const getTypeConfig = (type?: string) => {
    switch (type) {
      ...
      default:
        return { icon: MessageSquare, color: "text-slate-400", label: "Chat" };
    }
  };
...
const CodeSuggestionCard: React.FC<{
  ...
  return (
    <div className="border border-slate-700/50 rounded-lg overflow-hidden bg-slate-900/30 my-3 group hover:bg-slate-900/50 transition-colors">
...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Message', contextualMessage)
  ...
  return (
    <TooltipProvider>
      <>
        ...
        <div
          className={cn(
            "fixed right-0 top-0 h-full w-full max-w-6xl bg-slate-950 border-l border-slate-800 z-50 flex flex-col transition-transform duration-300 ease-out shadow-2xl",
        ...
        >
            ...
            <div className="absolute inset-0 bg-blue-500/20 border-2 border-dashed border-blue-400 z-10 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-blue-400 font-medium">
                  Drop MISRA C code files for analysis
                </p>
              </div>
            </div>
          )}
          ...
          {/* Enhanced Header */}
          <div className="shrink-0 border-b border-slate-800 bg-slate-900/80 backdrop-blur-sm">
            <div className="flex items-center justify-between p-6">
              <div className="flex items-center gap-3">
                <div className="relative w-14 h-14 border rounded-full flex flex-col justify-center items-center">
                  <Image src={"/ai-agent-chat.png"} alt="Logo" width={36} height={36} />
                </div>
                <div>
                  <h2 className="text-lg font-semibold text-slate-100">
                    Automotive Coding Assistant
                  </h2>
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="pl-7 h-8 w-40 bg-slate-800/50 border-slate-700/50"
                    />
                  </div>
            ...
            <div className="p-6 space-y-6">
              {filteredMessages.length === 0 && !isLoading && (
                <div className="text-center text-slate-500 py-16">
                  ...
                  <div className="grid grid-cols-2 gap-2 max-w-[80%] mx-auto">
                    {[
                      "Perform full MISRA C compliance check on the uploaded automotive code",
                      "Identify and correct rule violations for safety-critical ECU software",
                      "Optimize code for real-time automotive performance and memory efficiency",
                      "Enhance error handling in control modules following ISO 26262 guidelines",
                      "Suggest AI-driven improvements to reduce MISRA rule violations",
                      "Validate embedded C code for ECU resource constraints and reliability",
                      "Provide detailed MISRA C compliance report with recommendations",
                      "Refactor legacy automotive code for maintainability and safety compliance",
                      "Detect potential runtime issues in automotive control algorithms",
                      "Review code architecture for automotive standard compliance and efficiency"
                    ].map((suggestion) => (
                      <button
                        key={suggestion}
                        onClick={() => setInput(suggestion)}
                        className="px-3 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm text-slate-300 transition-colors text-left"
                      >
                        {suggestion}
                      </button>
                    ))}
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        ...
                        msg.role === "user"
                          ? "bg-slate-900/70 text-white p-4 rounded-br-md"
                          : "bg-slate-900/80 backdrop-blur-sm text-slate-100 p-5 rounded-bl-md border border-slate-800/50"
                      )}
                    >
                      {msg.role === "assistant" && (
              ...
              {isLoading && (
                <div className="flex items-start gap-4 justify-start">
                  ...
                  <div className="bg-slate-900/80 backdrop-blur-sm border border-slate-800/50 p-5 rounded-xl rounded-bl-md flex items-center gap-3">
                    <Loader2 className="h-4 w-4 animate-spin text-blue-400" />
                    <span className="text-sm text-slate-300">
                      {chatMode === "review"
                        ? "Performing MISRA C compliance analysis..."
                        : chatMode === "fix"
                        ? "Detecting rule violations and suggesting corrections..."
                        : chatMode === "optimize"
                        ? "Optimizing automotive code for performance and safety..."
                        : "Processing your automotive MISRA request..."}
                    </span>
                  </div>
                </div>
              )}
          ...
          <form
            ...
            className="shrink-0 p-4 border-t border-slate-800 bg-slate-900/80 backdrop-blur-sm"
          >
            <div className="flex items-end gap-3">
              <div className="flex-1 relative">
                <Textarea
                  placeholder={
                    chatMode === "chat"
                      ? "Paste MISRA C code or describe an automotive control issue..."
                      : chatMode === "review"
                  ...
                  disabled={isLoading}
                  className="min-h-[44px] max-h-32 bg-slate-800/50 border-slate-700/50 text-slate-100 placeholder-slate-500 focus:border-blue-500 focus:ring-blue-500/20 resize-none pr-20"
                  rows={1}
                />
                <div className="absolute right-3 bottom-3 flex items-center gap-2">
                  ...
                  <kbd className="hidden sm:inline-block px-1.5 py-0.5 text-xs text-slate-500 bg-slate-800 border border-slate-700 rounded">
                    ⌘↵
                  </kbd>
                </div>
              </div>
